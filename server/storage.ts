...
export class DatabaseStorage implements IStorage {
  ...
  // No default sample comps - system starts clean, comps must be imported from ATTOM
  ...
  private async getOrderComps(orderId: string): Promise<CompProperty[]> {
    if (!this.compsByOrder.has(orderId)) {
      ...
      if (compsFromFile.length > 0) {
        ...
        // No comps exist - start with empty array (comps must be imported from ATTOM)
        this.compsByOrder.set(orderId, []);
      }
    }
